...
- `wor_mining_pass_size` - номер строки пропуска в шахту (1-based, по умолчанию 1); используется в `Mining.processBuyPassPage()` для выбора нужного пропуска из таблицы `table.table_modern`
- и т.д.
...
- **Message passing** для взаимодействия с content scripts:
  - `sendRequestResolveCaptcha` - отправка капчи на внешний сервис. Данные: `{ bgBase64, puzzleBase64 }` — фон и кусочек пазла отдельными base64-изображениями (`puzzleBase64` может быть `null`)
...
- `cap.html` - страница капчи (пазл)
- `shaxtal.html` - страница шахты (покупка пропуска на добычу ресурсов)

При написании селекторов или поиске элементов **всегда сверяйся** с соответствующим файлом из этой папки.

### Нюансы HTML страницы боя

Страница боя (`boj.html`):

- **Имя и уровень** противника/игрока находятся в отдельных span'ах — `"Волот"` и `"8"`. Span'ы HP/MA имеют id (`hp_text`, `mana_text`) — исключаем через `:not([id])`. Блок игрока: `div[style*="user_hpma_frame"]`.
- **Тип удара** (`#udartype`, `#toggle-udartype`) сервер рендерит только если у игрока есть магическое оружие. Всегда проверяй наличие `#toggle-udartype` перед установкой значения.

### Ответ teritorystep.php

Каждый шаг по карте — запрос к `teritorystep.php`, возвращающий JSON. Ключевые поля:

```
ok          - 1 если успешно
room        - ID комнаты куда переместились
realUrl     - URL текущей страницы
grid        - массив клеток карты (9 колонок)
belowHtml    - актуальный HTML блока под картой (содержимое зависит от контекста — см. ниже)
headerHtml   - актуальный HTML блока над картой
players     - объект с позициями игроков на карте:
              { "209": { room: "209", kilk_player: "1", names: "Имя [уровень]" }, ... }
              names — строка через запятую, формат каждого: "Имя [уровень]"
anim        - настройки анимации { enabled, ms }
              ms = 0 в тактическом бою, ms = 50 на обычной карте
```

**Содержимое `belowHtml` зависит от контекста:**

- **Тактический бой** (`teritory-takt-teritorystep.json`): содержит состояние баз в формате `Название:<span ...>№N</span>` и списки игроков по командам (`Команда №N:` с ссылками на игроков). Используется в `TaktAuto.js` для определения захваченных баз (`shouldAbort`) и принадлежности игроков к командам (`getBlockedIds`).

- **Обычная карта** (`teritory-common-teritorystep.json`): содержит список игроков на текущей клетке и навигационное меню. Данных о базах и командах нет.
